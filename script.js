...
onload = function () {
    ...
    function createData(){
        ...
        for(let i=1;i<=sz;i++){
            for(let j=i+1;j<=sz;j++){
                ...
                if(Math.random() > 0.5){
                    ...
                    if(Math.random() > 0.5)
                        edges.push({from: i, to: j, label: String((10 + Math.floor(Math.random()*100/ 10)) * 10)});
                    else
                        edges.push({from: j, to: i, label: String((10 + Math.floor(Math.random()*100/ 10)) * 10)});
                }
            }
        }
        ...
        return data;
    } // this generates the adjacency list and returns the data.
...
// https://www.youtube.com/watch?v=Vy5PLlpDoks&list=PLfBJlB6T2eOu3dTPKzvAf2axlmQUXGY91&index=8